...
public:
    ...
    explicit ActionControl01(std::string name, const rclcpp::NodeOptions& node_options = rclcpp::NodeOptions()) : Node(name, node_options) {
    ...
    void result_callback(const GoalHandleMoveRobot::WrappedResult& result) {
        switch(result.code) {
            ...
            case rclcpp_action::ResultCode::CANCELED:
                ...
                return;
            default:
                RCLCPP_ERROR(this->get_logger(), "Unknown resutl code");
                return;
        }
